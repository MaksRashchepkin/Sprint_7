...
@allure.epic("Тестирование на получение заказа по номеру")
class TestGetOrder:
    order_methods = OrderMethods()

    @allure.title("Тест на успешное получение заказа по ID")
    @allure.description("Создание заказа и проверка, что можно успешно получить заказ по его ID.")
    ...
    @allure.description("Проверка, что запрос без номера заказа возвращает ошибку 400.")
    ...
    @allure.description("Проверка, что запрос с несуществующим заказом возвращает ошибку 404.")
